...
from transformers.models.big_bird.modeling_big_bird import BIG_BIRD_INPUTS_DOCSTRING
...
class BigBirdForXMC(BigBirdPreTrainedModel):
    """
    Examples:
        tokenizer = RobertaTokenizer.from_pretrained('roberta-base')
        model = RobertaForXMC.from_pretrained('roberta-base')
    ...
    def __init__(self, config):
        super(BigBirdForXMC, self).__init__(config)
        self.num_labels = config.num_labels
    ...
    def init_from(self, model):
        self.bigbird = model.bigbird

    @add_start_docstrings(BIG_BIRD_INPUTS_DOCSTRING.format("(batch_size, sequence_length)"))
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
...
ENCODER_CLASSES = {
    ...
    "big_bird": TransformerModelClass(BigBirdConfig, BigBirdForXMC, BigBirdTokenizerFast),
}
